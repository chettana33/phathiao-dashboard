...
**ภาพรวม:** vector search งานพาเที่ยว — corpus = Gochisou + PC + checkpoint + lessons + rules + SOP + ความรู้ทั้งระบบ · store local `vec_test` · เสร็จ **8/8 ส่วน** · default = **local bge-m3** (6 ก.ย. 69 พี่เจเลือก)
...
| 7 | ขยาย corpus ครอบทั้งระบบ | 100% | done | พาเที่ยว | รอบ 1-4 (7 ก.ย.): 314 → 506 → 615+ → **689** → +SOP 02_Operations + protocols — AGENTS/3LAYER/locator/indexes/memories/SOP/workflow_tree/**rules/BOOT**/ops-SOP + secret gate 0 รั่ว · store `patheaw_work` ~750 (ดู chroma_status) |
| 8 | auto-load ใน `/work` | 100% | done | พาเที่ยว | hint layer ทำแล้ว 7 ก.ย.: chunk มี metadata `section` + search/CLI/MCP คืน {src, section} → skill /work ขั้น 8 = เปิดไฟล์จริงจาก hint (ห้ามพึ่ง snippet); store 836 chunks |
